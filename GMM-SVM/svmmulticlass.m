...
%
%		for more details see new_GMMkernel
% 
...
%
%
% see also svmreg, new_GMMkernel, svmval
...
for s=1:nbclass
    ...
    MatAux(debut:debut+ellinclass-1,debut:debut+ellinclass-1)=new_GMMkernel(x(indclass,:),kerneloption);
    debut=debut+ellinclass;
    
    MatAuxQ3(debutQ3:debutQ3+ell-1,debutQ3:debutQ3+ell-1)=new_GMMkernel(x,kerneloption);
    debutQ3=debutQ3+ell;   
    for j=1:ell
        Kij=new_GMMkernel(x,kerneloption,x(j,:));
        yj=y(j);
        Q21( (yj-1)*ell +1: yj *ell, (s-1)*ell + j )= -Kij; 
        
        ind=find(yextended==s);
        Q22(ind,(s-1)*ell + j )= - new_GMMkernel(xextended(ind,:),kerneloption,x(j,:));
