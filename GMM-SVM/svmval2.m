function [y,y1,y2]=svmval(x,xsup,w,b,kernel,kerneloption,V,span,framematrix,vector,dual)
...
if ~strcmp(kernel,'frame') | nargin<9; %8
    framematrix=[];
    vector=[];
end;
if nargin <11 %10
...
elseif isfield(xsup,'datafile') | isfield(x,'datafile');  % data is stored in file and not in memory
    ...
    %ps=svmkernel(x,kernel,kerneloption,xsup,framematrix,vector,dual);
     %ps = GMMkernel(x',kerneloption,xsup');
     ps = new_GMMkernel(x,xsup,kerneloption,V);
      %ps = new_GMMkernel_mcov(x,kerneloption,xsup,V);
     %ps = BhattKernel(x,xsup, V);
     %ps = GMMkernel_mcov(x',kerneloption,xsup');
      %ps = cosine_kernel(x,xsup, V);
