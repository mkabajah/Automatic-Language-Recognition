function [xsup,w,d,pos,timeps,alpha,obj]=svmclass(x,y,c,lambda,kernel,kerneloption,verbose,U,span, alphainit)
...
%ps = GMMkernel(x',kerneloption);
ps = new_GMMkernel(x,x,kerneloption,U);
 %ps = new_GMMkernel_mcov(x,kerneloption,x,U);
%ps = BhattKernel(x,x,U);
%ps = GMMkernel_mcov(x',kerneloption);
 %ps = cosine_kernel(x,x,U);
